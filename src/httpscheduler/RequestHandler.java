    ...
    public void handle(
        ...
        if (request instanceof HttpEntityEnclosingRequest) {
            ...
            if (requestArgs.size() == 3) {
                assert requestArgs.containsKey("job-id") && requestArgs.containsKey("tasks") && requestArgs.containsKey("task-ids");
    
                String jobID = requestArgs.get("job-id");
                String tasksToSend = requestArgs.get("tasks");
                String taskIDs = requestArgs.get("task-ids");
                System.out.println(jobID);
                System.out.println(tasksToSend);
                System.out.println(taskIDs);
                // replace all + with spaces
                //taskToProcess = taskToProcess.replaceAll("\\+", " ");
                //Thread taskCommExecutorThread = new TaskCommExecutorThread(taskToProcess);
                //Future threadMonitor = taskCommExecutor.submit(taskCommExecutorThread);
